...
class ChessAI():
    ...
    def findBestChess(self, board, turn):
        ...
        time2 = time.time()
        DEBUG(DEBUG_WARN, 'time[%.2f] %d(%d, %d), score[%d] alpha[%d] belta[%d] save[%d] cache[%d]' % (
        (time2 - time1), self.number, x, y, score, self.alpha, self.belta, self.save_count, self.cacheGet))
        return (x, y)
    ...
    def analysisLine(self, board, x, y, dir_index, dir, mine, opponent):
        ...
        type_list = [type_dir1, type_dir2, type_dir3, type_dir4, type_dir5, type_dir6, type_dir7, type_dir8, type_dir9,
                     type_dir10]
        if self.checTypeList(board, x, y, dir, type_list, opponent):
