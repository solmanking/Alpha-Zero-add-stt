...
class Map():
    def __init__(self, width, height):  # 輸入的width*height 轉為矩陣
        self.width = width
        self.height = height
        self.map = [[0 for x in range(self.width)] for y in range(self.height)]

        self.map[3][2]=1     #測試如果值為1

        print(self.map)                                     # map =8*8的陣列
        self.steps = []

    def reset(self):  ##重整棋盤 將 x(height) * y(width)的陣列 歸零
        for y in range(self.height):
            for x in range(self.width):
                self.map[y][x] = 0
        self.steps = []

   # def reverseTurn(self, turn):  ##回合轉換 玩家1~2
       # if turn == MAP_ENTRY_TYPE.MAP_PLAYER_ONE:
          #  return MAP_ENTRY_TYPE.MAP_PLAYER_TWO
       # else:
         #   return MAP_ENTRY_TYPE.MAP_PLAYER_ONE

    def getMapUnitRect(self, x, y):  # (map_x,map_y)是旗子的位置 得到地圖單位
        map_x = x * REC_SIZE
        map_y = y * REC_SIZE

        return (map_x, map_y, REC_SIZE, REC_SIZE)

    def MapPosToIndex(self, map_x, map_y):  # 旗子坐落的位置
        x = map_x // REC_SIZE
        y = map_y // REC_SIZE
        return (x, y)

    def isInMap(self, map_x, map_y):  # max_xy必須介於0~15之間
    ...
    def Input(self, x, y,type): ##嘗試增加input法
        self.map[y][x] = type.value
        self.steps.append((x,y))



    def drawChess(self, screen):  # 劃出旗子
    ...
    def drawBackground(self, screen):  ##背景
        color = (0, 0, 0)
        for y in range(self.height):
