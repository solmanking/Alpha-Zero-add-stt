...
class Game():
    ...
    def start(self):  # 遊戲開始時
        ...
        self.AI.number = 0
        if (self.mode == USER_VS_AI_MODE and self.AI_first) or self.mode == AI_VS_AI_MODE:  # 確認ai是否加入對戰
            self.useAI = True
        else:
            self.useAI = False
    ...
    def play(self):
        ...
        pygame.draw.rect(self.screen, light_yellow, pygame.Rect(0, 0, MAP_WIDTH, SCREEN_HEIGHT))  # 繪製正方形
        ...
        if self.is_play and not self.isOver():                                           #遊戲開始時
            if self.useAI:
    ...
    def inputPos(self, map_x,map_y):       #新增 input方法

        map_xy= input('請輸入座標:')
        if isinstance(map_xy, str):  # for python3 #如果lacation屬性是string
            location = [int(n, 10) for n in map_xy.split(",")]  # 將字串轉成[x,y]
            x=location[0]
            y=location[1]
            self.action =(x, y)
